    ...
    registerHandlers(): void {
        ...
        this._fuseMockApiService
            ...
            .reply(({ request }) => {
                ...
                const index = this._calendars.findIndex(
        ...
        this._fuseMockApiService
            ...
            .reply(({ request }) => {
                ...
                const viewStart = moment(request.params.get('start')).startOf(
                    'day',
                );
                ...
                events.forEach(event => {
                    ...
                    if (!event.recurrence) {
                        ...
                        if (
                            eventStart.isSameOrAfter(viewStart, 'day')
                            && eventEnd.isSameOrBefore(viewEnd, 'day')
                        ) {
                    ...
                    // If it's a recurring event...
                        ...
                        if (
                            eventStart.isAfter(viewEnd, 'day')
                            || eventEnd.isBefore(viewStart, 'day')
                        ) {
                            return;
                        }
        ...
        this._fuseMockApiService
            ...
            .reply(({ request }) => {
                ...
                const recurringEvent = this._events.find(
                    item => item.id === event.recurringEventId,
                );
                ...
                if (mode === 'single') {
                    ...
                    newEvent.end = moment(newEvent.start).add(
                        newEvent.duration,
                        'minutes',
                    );
                    ...
                    if (originalEvent.start === recurringEvent.start) {
                        ...
                        const ruleDate = moment(
                            rruleset.all((date, i) => i < 2)[1],
                        );
                ...
                if (mode === 'future') {
                    // Update the end date
                    recurringEvent.end = moment(originalEvent.start)
                        .subtract(1, 'day')
                        .endOf('day')
                        .toISOString();
                    ...
                    parsedRules['UNTIL'] = moment(recurringEvent.end)
                        .utc()
                        .format('YYYYMMDD[T]HHmmss[Z]');
        ...
        this._fuseMockApiService
            ...
            .reply(({ request }) => {
                ...
                if (mode === 'future') {
                    ...
                    recurringEvent.end = moment(event.start)
                        .subtract(1, 'day')
                        .endOf('day')
                        .toISOString();
                    ...
                    // Add/Update the UNTIL rule
                    parsedRules['UNTIL'] = moment(event.end)
                        .utc()
                        .format('YYYYMMDD[T]HHmmss[Z]');
    ...
    private _generateRuleset(
        ...
        this._exceptions.forEach(item => {
            ...
            if (item.eventId === event.id) {
                ...
                ruleSet.push(
                    'EXDATE:'
                        + moment(item.exdate).format('YYYYMMDD[T]HHmmss[Z]'),
                );
            }
        });
